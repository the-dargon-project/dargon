...
namespace Dargon.Commons.Utilities {
   public static partial class BinarySearchExtensions {
      public static int BinarySearchPredicate<T, TPredicate>(this List<T> items, TPredicate predicate)
         where TPredicate : struct, ITemplatePredicate<T> {
         return BinarySearchPredicate<T, TPredicate>(items, 0, items.Count - 1, predicate);
      }

      public static int BinarySearchPredicate<T, TPredicate>(this List<T> items, int lo, int hi, TPredicate predicate)
         where TPredicate : struct, ITemplatePredicate<T> {
         while (lo < hi) {
            var mid = lo + (hi - lo) / 2;
      ...
      public static int BinarySearchPredicate<T>(this List<T> items, Func<T, bool> predicate) {
         return BinarySearchPredicate(items, 0, items.Count - 1, predicate);
      }

      public static int BinarySearchPredicate<T>(this List<T> items, int lo, int hi, Func<T, bool> predicate) {
         while (lo < hi) {
            var mid = lo + (hi - lo) / 2;
